...
# KDD'24 Submission: DimCL_layer


class FinalSimDimCL(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(FinalSimDimCL, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['FinalSimDimCL'])
        ...
        self.model = FinalSimDimCL_Encoder(self.data, self.emb_size, self.temp, self.taus, self.eps, self.n_layers,
                                      self.device)
...
class FinalSimDimCL_Encoder(nn.Module):
    def __init__(self, data, emb_size, temp, taus, eps, n_layers, device):
        super(FinalSimDimCL_Encoder, self).__init__()
        self.data = data
        self.eps = eps
        self.d = emb_size
    ...
    def forward(self, uids=0, pos=0, neg=0, perturbed=False):
        ...
        if perturbed:
            ...
            for k in range(self.n_layers):
                ...
                random_noise1 = torch.rand_like(ego_embeddings).cuda()
                random_noise2 = torch.rand_like(ego_embeddings).cuda()
                ego_embeddings1 = ego_embeddings + torch.sign(ego_embeddings) * F.normalize(random_noise1,
                                                                                            dim=-1) * self.eps
                ego_embeddings2 = ego_embeddings + torch.sign(ego_embeddings) * F.normalize(random_noise2,
                                                                                            dim=-1) * self.eps
                user_emb1, item_emb1 = torch.split(ego_embeddings1, [self.data.user_num, self.data.item_num])
                user_emb2, item_emb2 = torch.split(ego_embeddings2, [self.data.user_num, self.data.item_num])
                views_user, views_item = self.augmentation_simgcl(user_emb1, item_emb1, user_emb2, item_emb2, uids, pos,
                                                                  neg)
                user_view1, user_view2 = views_user
                item_view1, item_view2 = views_item
                view1_embs = torch.concat((user_view1, item_view1), dim=0)
                view2_embs = torch.concat((user_view2, item_view2), dim=0)
                all_embs_1.append(view1_embs)
                all_embs_2.append(view2_embs)

                ego_embeddings += 0.5 * torch.sign(ego_embeddings) * F.normalize(random_noise1,
                                                                                 dim=-1) * self.eps + 0.5 * torch.sign(
                    ego_embeddings) * F.normalize(random_noise2,
                                                  dim=-1) * self.eps
                ...
                # views_user, views_item = self.augmentation(user_emb, item_emb, uids, pos, neg)
                # user_view1, user_view2 = views_user
                # item_view1, item_view2 = views_item
                # view1_embs = torch.concat((user_view1, item_view1), dim=0)
                # view2_embs = torch.concat((user_view2, item_view2), dim=0)
                # all_embs_1.append(view1_embs)
                # all_embs_2.append(view2_embs)
            all_embs_1 = torch.stack(all_embs_1, dim=1)
            all_embs_1 = torch.mean(all_embs_1, dim=1)
            all_embs_2 = torch.stack(all_embs_2, dim=1)
            all_embs_2 = torch.mean(all_embs_2, dim=1)
            user_v1, item_v1 = torch.split(all_embs_1, [self.data.user_num, self.data.item_num])
            user_v2, item_v2 = torch.split(all_embs_2, [self.data.user_num, self.data.item_num])
    ...
    def generate_views(self, embs, noises, w=(0, 0), type='ge'):
        noise1, noise2 = noises
        if type == 'ge':
            noise1 = torch.sign(embs) * F.normalize(noise1) * self.eps
            noise2 = torch.sign(embs) * F.normalize(noise2) * self.eps
            view1 = embs + noise1
            view2 = embs + noise2
        elif type == 'de':
            noise1 = torch.sign(embs) * F.normalize(noise1) * self.eps
