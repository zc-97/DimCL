...
class NCL(GraphRecommender):
    ...
    def e_step(self):
        ...
        # user_embeddings = self.model.embedding_dict['user_emb']
        # item_embeddings = self.model.embedding_dict['item_emb']
    ...
    #     # Convert to cuda Tensors for broadcast
    #     centroids = torch.Tensor(cluster_cents).cuda()
    #     node2cluster = torch.LongTensor(labels).cuda()
    #
    #     return centroids, node2cluster

    def run_kmeans(self, x, num_iters=100):
        """Run K-means algorithm to get k clusters of the input tensor x using PyTorch for GPU acceleration"""
        ...
        for i in range(num_iters):
            ...
            # Update centroids
            new_centroids = torch.stack([x[labels == j].mean(dim=0) for j in range(self.k)])
